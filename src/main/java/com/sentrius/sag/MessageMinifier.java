...
public class MessageMinifier {
    ...
    private static String minifyStatement(Statement stmt) {
        ...
        } else if (stmt instanceof FoldStatement) {
    ...
    private static String minifyFold(FoldStatement fold) {
        StringBuilder sb = new StringBuilder();
        sb.append("FOLD ").append(fold.getFoldId());
        sb.append(" \"").append(escapeString(fold.getSummary())).append("\"");
        if (fold.getState() != null) {
            sb.append(" STATE ").append(minifyValue(fold.getState()));
        }
        return sb.toString();
    }
